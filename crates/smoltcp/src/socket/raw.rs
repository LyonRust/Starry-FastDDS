...
impl<'a> Socket<'a> {
    ...
    pub fn send_slice(&mut self, data: &[u8]) -> Result<(), SendError> {
        // axlog::error!("--- send_slice: {:?}", data);
        // let a = self.send(data.len())?;
        // axlog::error!("--- send_slice a : {:?}", a);
        // a.copy_from_slice(data);
        // axlog::error!("--- haha {:?}", a);

        self.send(data.len())?.copy_from_slice(data);

        axlog::error!("--- self.tx_buffer {:?}", self.tx_buffer.is_empty());
        Ok(())
    }
    ...
    pub fn recv_slice(&mut self, data: &mut [u8]) -> Result<usize, RecvError> {
        axlog::error!("recv_slice : {}",  self.rx_buffer.is_empty());
